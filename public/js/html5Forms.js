					...
					switch(supportReq) {
								...
								if (!Modernizr.input.required || hasBadValidationImplementation || me.forceJSValidation) {
									...
									if (isScriptCompressed) {
										toLoad = toLoad.concat([  
											scriptDir + 'weston.ruter.net/webforms2/webforms2-p.js']);
									} else {
										toLoad = toLoad.concat([  
											scriptDir + 'weston.ruter.net/webforms2/webforms2_src.js']);
									}
						...
						case "color":
							if (!inputSupport.color) {
								...
								toLoad = toLoad.concat([  scriptDir + 'jscolor/jscolor.js']);
						...
						case "range":
						   /* yepnope({
							...
							if(!inputSupport.range) {
								toLoad = toLoad.concat([  scriptDir + 'css/slider.css',
										  scriptDir + 'frequency-decoder.com/slider.js']);
